const i18n = {
  ...
  en: {
    ...
    detail: {
      ...
      imageMoveUp: "Move Up One Line",
      imageMoveDown: "Move Down One Line",
      crop: "Crop",
      widthPx: "Width (px)",
      heightPx: "Height (px)",
...
const state = {
  ...
  projectListDraggingId: "",
  projectListDropTargetId: "",
  projectListDropPlacement: "before",
  projectListReordering: false,
  projectListSuppressOpenUntil: 0,
  lineNumberRaf: 0
};
...
const elements = {
  ...
  imageMoveUp: document.getElementById("image-move-up"),
  imageMoveDown: document.getElementById("image-move-down"),
...
if (elements.projectList) {
  elements.projectList.addEventListener("dragover", handleProjectListDragOver);
  elements.projectList.addEventListener("drop", (event) => {
    void handleProjectListDrop(event);
  });
}
...
if (elements.personalizeFooterEditor) {
  ...
  elements.personalizeFooterEditor.addEventListener("paste", (event) => {
    handleEditorPasteWithUnifiedFont(event, "personalize");
  });
...
if (elements.aboutEditor) {
  ...
  elements.aboutEditor.addEventListener("paste", (event) => {
    handleEditorPasteWithUnifiedFont(event, "about");
  });
...
elements.detailEditor.addEventListener("paste", (event) => {
  ...
  if (html && html.includes("data-editor-copied-image")) {
    const inserted = insertCopiedImageWrappersFromClipboardHtml(html);
    if (!inserted) return;
    event.preventDefault();
    return;
  }
  handleEditorPasteWithUnifiedFont(event, "detail");
});
...
if (elements.imageMoveUp) {
  elements.imageMoveUp.addEventListener("click", () => moveSelectedImageByLine("up"));
}
if (elements.imageMoveDown) {
  elements.imageMoveDown.addEventListener("click", () => moveSelectedImageByLine("down"));
}
...
function normalizePastedHtmlTypography(html) {
  const source = String(html || "").trim();
  if (!source) return "";
  const probe = document.createElement("div");
  probe.innerHTML = source;

  const nodes = [probe, ...Array.from(probe.querySelectorAll("*"))];
  nodes.forEach((node) => {
    ...
    const hasFontShorthand = Boolean(style.getPropertyValue("font"));
    if (hasFontShorthand) {
      ...
      if (fontStyle && fontStyle !== "normal") {
        style.setProperty("font-style", fontStyle, fontPriority);
      }
      if (fontVariant && fontVariant !== "normal") {
        style.setProperty("font-variant", fontVariant, fontPriority);
      }
      if (fontWeight && fontWeight !== "normal" && fontWeight !== "400") {
        style.setProperty("font-weight", fontWeight, fontPriority);
      }
      if (fontStretch && fontStretch !== "normal") {
        style.setProperty("font-stretch", fontStretch, fontPriority);
      }
      if (fontSize) {
        style.setProperty("font-size", fontSize, fontPriority);
      }
      if (lineHeight && lineHeight !== "normal") {
        style.setProperty("line-height", lineHeight, fontPriority);
      }
    }
...
function handleEditorPasteWithUnifiedFont(event, context) {
  if (!event || !event.clipboardData) return false;
  const html = event.clipboardData.getData("text/html");

  if (html) {
    const normalizedHtml = normalizePastedHtmlTypography(html);
    if (normalizedHtml) {
      event.preventDefault();
      runCommandForContext(context, "insertHTML", normalizedHtml);
...
function getRangeListByLangWithFallback(rangesByLang, lang, templatePresenceByLang = null) {
  const currentLang = lang === "en" ? "en" : "zh";
  const fallbackLang = getOtherLang(currentLang);
  const primary = normalizeLineRanges(rangesByLang?.[currentLang] || []);
  if (primary.length) return primary;
  if (
    templatePresenceByLang
    && Object.prototype.hasOwnProperty.call(templatePresenceByLang, currentLang)
    && templatePresenceByLang[currentLang]
  ) {
    return primary;
  }
...
function getRangesForContext(context) {
  if (context === "detail") {
    return getRangeListByLangWithFallback(
      state.detailBgRanges,
      state.detailLang,
      state.detailBgRangeTemplatePresence
    );
  }
  if (context === "about") {
...
function setRangesForContext(context, nextRanges) {
  ...
  if (context === "detail") {
    ...
    state.detailBgRangeTemplatePresence[state.detailLang] = true;
    return;
  }
  if (context === "about") {
...
function getTotalEditorLineCount(context) {
  const { editor } = getEditorAndGutterByContext(context);
  if (!(editor instanceof HTMLElement)) return 1;
  const count = getLineEntriesForEditor(context, editor).length;
  return Math.max(1, count || 1);
}

function getClampedRangesForContext(context, totalLines) {
  const safeTotal = Number.isFinite(totalLines) ? Math.max(1, Math.floor(totalLines)) : 1;
  const ranges = getRangesForContext(context);
  return normalizeLineRanges(ranges.map((range) => {
    const rawStart = Number.parseInt(range?.start, 10);
    const rawEnd = Number.parseInt(range?.end, 10);
    const startBase = Number.isFinite(rawStart) ? rawStart : 1;
    const endBase = Number.isFinite(rawEnd) ? rawEnd : startBase;
    const start = Math.min(Math.max(1, Math.min(startBase, endBase)), safeTotal);
    const end = Math.min(Math.max(start, Math.max(startBase, endBase)), safeTotal);
    return {
      start,
      end,
      color: normalizeHexColor(range?.color || "#f5f2ed")
    };
  }));
}
...
function openLineRangePopover(context, triggerButton) {
  ...
  const lineCount = getTotalEditorLineCount(context);
  const line = Math.min(getSelectionLineNumber(context), lineCount);
  ...
  if (elements.lineRangeColor) {
    const ranges = getClampedRangesForContext(context, lineCount);
...
function applyLineRangeFromPopover() {
  ...
  const totalLines = getTotalEditorLineCount(state.lineRangeContext);
  ...
  const startSafe = Math.min(Math.max(1, startRaw), totalLines);
  const endSafe = Math.min(Math.max(1, endRaw), totalLines);
  ...
  const nextRanges = getClampedRangesForContext(state.lineRangeContext, totalLines)
...
function getProjectRowFromNode(node) {
  const row = node instanceof Element ? node.closest("tr.project-row[data-project-id]") : null;
  return row instanceof HTMLTableRowElement ? row : null;
}

function isProjectRowPending(row) {
  return row instanceof HTMLElement && row.dataset.pending === "1";
}

function clearProjectListDropMarkers() {
  if (!elements.projectList) return;
  elements.projectList.querySelectorAll(".project-row.is-drag-over-before, .project-row.is-drag-over-after")
    .forEach((row) => {
      row.classList.remove("is-drag-over-before", "is-drag-over-after");
    });
}

function setProjectListDropMarker(row, placement) {
  clearProjectListDropMarkers();
  if (!(row instanceof HTMLTableRowElement)) {
    state.projectListDropTargetId = "";
    state.projectListDropPlacement = "before";
    return;
  }
  if (!row.dataset.projectId || isProjectRowPending(row)) {
    state.projectListDropTargetId = "";
    state.projectListDropPlacement = "before";
    return;
  }
  const normalizedPlacement = placement === "after" ? "after" : "before";
  row.classList.add(normalizedPlacement === "after" ? "is-drag-over-after" : "is-drag-over-before");
  state.projectListDropTargetId = row.dataset.projectId;
  state.projectListDropPlacement = normalizedPlacement;
}

function resetProjectListDragState() {
  state.projectListDraggingId = "";
  state.projectListDropTargetId = "";
  state.projectListDropPlacement = "before";
  clearProjectListDropMarkers();
  if (!elements.projectList) return;
  elements.projectList.querySelectorAll(".project-row.is-dragging").forEach((row) => {
    row.classList.remove("is-dragging");
  });
}

function handleProjectRowDragStart(event, row, projectId) {
  if (state.projectListReordering || !projectId) {
    event.preventDefault();
    return;
  }
  const target = event.target instanceof Element ? event.target : null;
  ...
  state.projectListSuppressOpenUntil = Date.now() + 220;
  state.projectListDraggingId = projectId;
  state.projectListDropTargetId = "";
  state.projectListDropPlacement = "before";
  ...
  if (event.dataTransfer) {
    ...
    try {
      ...
      // Ignore browsers that block drag payload writes.
    }
  }
}

function handleProjectRowDragEnd() {
  resetProjectListDragState();
}

function handleProjectListDragOver(event) {
  if (!state.projectListDraggingId || state.projectListReordering) return;
  ...
  const row = getProjectRowFromNode(event.target);
  if (!row || isProjectRowPending(row) || row.dataset.projectId === state.projectListDraggingId) {
    setProjectListDropMarker(null, "before");
    return;
  }

  const rect = row.getBoundingClientRect();
  const placeAfter = event.clientY > rect.top + rect.height / 2;
  setProjectListDropMarker(row, placeAfter ? "after" : "before");
}

async function reorderProjectListByDrag(draggedId, targetId, placement = "before") {
  const dragId = String(draggedId || "").trim();
  const dropId = String(targetId || "").trim();
  if (!dragId || !dropId || dragId === dropId) return;

  const beforeOrder = getProjectsSortedForDisplay(state.projects);
  const fromIndex = beforeOrder.findIndex((project) => project.id === dragId);
  const toIndex = beforeOrder.findIndex((project) => project.id === dropId);
  if (fromIndex === -1 || toIndex === -1) return;

  const afterOrder = [...beforeOrder];
  const [draggedProject] = afterOrder.splice(fromIndex, 1);
  if (!draggedProject) return;

  let insertIndex = toIndex + (placement === "after" ? 1 : 0);
  if (fromIndex < insertIndex) {
    insertIndex -= 1;
  }
  insertIndex = Math.max(0, Math.min(insertIndex, afterOrder.length));
  afterOrder.splice(insertIndex, 0, draggedProject);

  const beforeIds = beforeOrder.map((project) => project.id);
  const afterIds = afterOrder.map((project) => project.id);
  if (beforeIds.length === afterIds.length && beforeIds.every((id, index) => id === afterIds[index])) {
    return;
  }

  const previousRanks = new Map(
    state.projects.map((project) => [project.id, normalizeProjectRank(project.rank)])
  );
  afterOrder.forEach((project, index) => {
    project.rank = String(index + 1);
  });

  state.projectListReordering = true;
  try {
    await writeProjectsCsv();
    if (state.currentId && elements.rank) {
      const current = state.projects.find((project) => project.id === state.currentId);
      elements.rank.value = current ? normalizeProjectRank(current.rank) : "";
    }
    renderProjectList();
    scrollProjectRowIntoView(dragId);
    state.projectListSuppressOpenUntil = Date.now() + 260;
    setStatus(t("status.saved"));
  } catch (error) {
    state.projects.forEach((project) => {
      project.rank = previousRanks.get(project.id) || "";
    });
    renderProjectList();
    setStatus(t("status.error").replace("{message}", error.message), "error");
  } finally {
    state.projectListReordering = false;
  }
}

async function handleProjectListDrop(event) {
  if (!state.projectListDraggingId || state.projectListReordering) return;
  event.preventDefault();

  let targetId = state.projectListDropTargetId;
  let placement = state.projectListDropPlacement || "before";
  const targetRow = getProjectRowFromNode(event.target);
  if (targetRow && !isProjectRowPending(targetRow) && targetRow.dataset.projectId) {
    if (targetRow.dataset.projectId !== state.projectListDraggingId) {
      const rect = targetRow.getBoundingClientRect();
      const placeAfter = event.clientY > rect.top + rect.height / 2;
      targetId = targetRow.dataset.projectId;
      placement = placeAfter ? "after" : "before";
    }
  }

  if (!targetId || targetId === state.projectListDraggingId) {
    resetProjectListDragState();
    return;
  }

  try {
    await reorderProjectListByDrag(state.projectListDraggingId, targetId, placement);
  } finally {
    resetProjectListDragState();
  }
}
  ...
  rows.forEach((project) => {
    ...
    if (!project._isPending) {
      ...
      row.addEventListener("dragstart", (event) => {
        handleProjectRowDragStart(event, row, project.id);
      });
      row.addEventListener("dragend", () => {
        handleProjectRowDragEnd();
      });
    }
    row.innerHTML = `
      <td>${project.id}</td>
    ...
    if (!project._isPending) {
      row.addEventListener("click", async (event) => {
        ...
        if (state.projectListReordering || Date.now() < state.projectListSuppressOpenUntil) return;
...
function parseDetailContentPage(html) {
  ...
  try {
    ...
    const hasZhRangeTemplate = Boolean(zhRangesTemplate);
    const hasEnRangeTemplate = Boolean(enRangesTemplate);
    const zhRanges = parseLineRangesTemplateText(zhRangesTemplate ? zhRangesTemplate.innerHTML : "");
    const enRanges = parseLineRangesTemplateText(enRangesTemplate ? enRangesTemplate.innerHTML : "");
    if (zhTemplate || enTemplate) {
      return {
        ...
        bgRangeTemplatePresence: {
          zh: hasZhRangeTemplate,
          en: hasEnRangeTemplate
        }
      };
    }
...
function pasteDetailFromOtherLang() {
  ...
  state.detailBgRangeTemplatePresence[targetLang] = true;
  renderDetailEditor();
}
...
async function openDetailEditor(id) {
  ...
  try {
    ...
    if (html) {
      ...
      state.detailBgRangeTemplatePresence.zh = Boolean(parsed.bgRangeTemplatePresence?.zh);
      state.detailBgRangeTemplatePresence.en = Boolean(parsed.bgRangeTemplatePresence?.en);
    } else {
      ...
      state.detailBgRangeTemplatePresence.zh = false;
      state.detailBgRangeTemplatePresence.en = false;
    }
  } catch (error) {
    setStatus(t("status.error").replace("{message}", error.message), "error");
    state.detailImportedMode = false;
...
async function saveDetailHtml(project, contentInput) {
  ...
  const resolvedEmbeddedImageSrc = new Map();
  ...
  resolvedZh = await resolveEmbeddedDataImages(resolvedZh, project, resolvedEmbeddedImageSrc);
  ...
  resolvedEn = await resolveEmbeddedDataImages(resolvedEn, project, resolvedEmbeddedImageSrc);
  ...
  state.detailBgRangeTemplatePresence.zh = true;
  state.detailBgRangeTemplatePresence.en = true;
  renderDetailEditor();
  await migrateLegacyDetail(project);
}
...
function resolveImageDropAnchorNode(node) {
  if (!(node instanceof Node) || !(elements.detailEditor instanceof HTMLElement)) return null;
  let anchor = node;
  if (anchor instanceof Text) {
    if (anchor.parentNode === elements.detailEditor) {
      return anchor;
    }
  ...
  const disallowedParentTags = new Set(["UL", "OL", "TABLE", "TBODY", "THEAD", "TFOOT", "TR"]);
  const stableBlockTags = new Set([
    "P",
    "DIV",
    "SECTION",
    "ARTICLE",
  ...
  while (anchor instanceof HTMLElement && anchor.parentNode && anchor.parentNode !== elements.detailEditor) {
    const parent = anchor.parentNode;
    if (!(parent instanceof HTMLElement)) break;
    if (parent.classList.contains("detail-columns-cell")) break;
    if (disallowedParentTags.has(parent.tagName)) {
      anchor = parent;
      continue;
    }
    if (stableBlockTags.has(anchor.tagName)) {
      break;
    }
    anchor = parent;
  }

  if (!(anchor instanceof Node) || anchor === elements.detailEditor || !elements.detailEditor.contains(anchor)) {
    return null;
  }
  return anchor;
}

function createRangeAroundLineEntryAnchor(lineEntry, placement = "after") {
  if (!lineEntry || !(lineEntry.anchorNode instanceof Node)) return null;
  const anchor = resolveImageDropAnchorNode(lineEntry.anchorNode);
  if (!(anchor instanceof Node)) return null;
  const range = document.createRange();
  try {
    if (anchor instanceof HTMLElement && anchor.classList.contains("detail-columns-cell")) {
      range.selectNodeContents(anchor);
      range.collapse(placement !== "after");
...
async function resolveEmbeddedDataImages(contentHtml, project, resolvedBySource = null) {
  const container = document.createElement("div");
  container.innerHTML = contentHtml;
  ...
  for (const img of images) {
    ...
    if (resolvedBySource instanceof Map && resolvedBySource.has(rawSrc)) {
      img.setAttribute("src", resolvedBySource.get(rawSrc));
      continue;
    }
    ...
    try {
      ...
      const mimeMatch = rawSrc.match(/^data:([^;,]+)[;,]/i);
      const mimeType = mimeMatch ? mimeMatch[1] : blob.type;
      const ext = getFileExtensionFromMimeType(mimeType);
      const file = new File([blob], `embedded.${ext}`, { type: mimeType || blob.type || `image/${ext}` });
      const src = await saveDetailImage(project, file);
      if (!src) {
        failedCount += 1;
        continue;
      }
      img.setAttribute("src", src);
      if (resolvedBySource instanceof Map) {
        resolvedBySource.set(rawSrc, src);
      }
    } catch (error) {
      failedCount += 1;
    }
  ...
  if (failedCount) {
    setStatus(t("status.error").replace("{message}", "Some embedded images failed to save"), "error");
  }
...
function syncImageMoveButtons(wrapper) {
  if (elements.imageMoveUp) {
    elements.imageMoveUp.disabled = true;
  }
  if (elements.imageMoveDown) {
    elements.imageMoveDown.disabled = true;
  }
  if (!(wrapper instanceof HTMLElement) || !(elements.detailEditor instanceof HTMLElement)) return;
  if (!elements.detailEditor.contains(wrapper)) return;

  const lineEntries = getLineEntriesForEditor("detail", elements.detailEditor);
  if (!lineEntries.length) return;
  const currentIndex = lineEntries.findIndex(
    (entry) => entry.anchorNode === wrapper || entry.nodes.includes(wrapper)
  );
  if (currentIndex < 0) return;

  if (elements.imageMoveUp) {
    elements.imageMoveUp.disabled = currentIndex <= 0;
  }
  if (elements.imageMoveDown) {
    elements.imageMoveDown.disabled = currentIndex >= lineEntries.length - 1;
  }
}
...
function moveSelectedImageByLine(direction) {
  const wrapper = state.selectedImage;
  if (!(wrapper instanceof HTMLElement) || !(elements.detailEditor instanceof HTMLElement)) return false;
  if (!elements.detailEditor.contains(wrapper)) return false;

  const lineEntries = getLineEntriesForEditor("detail", elements.detailEditor);
  if (!lineEntries.length) return false;
  ...
  if (targetIndex < 0 || targetIndex >= lineEntries.length) return false;

  const targetEntry = lineEntries[targetIndex];
  const insertionRange = createRangeAroundLineEntryAnchor(
    targetEntry,
    normalizedDirection === "up" ? "before" : "after"
  );
  if (!(insertionRange instanceof Range)) return false;
